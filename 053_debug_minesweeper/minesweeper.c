...
int click(board_t * b, int x, int y) {
  if (x < 0 || x >= b->width || y < 0 || y >= b->height) {  // swich >= to >
...
int checkWin(board_t * b) {
  // it's stopping at the first square since it is not unknown
  // get max squares = int criteria = b->width * b->height
  int criteria = b->width * b->height;
  int counter = 0;
  // when the counter is satisfied, return 1
...
int maybeReveal(board_t * b, int x, int y) {
  ...
  int knownMines = 0;
  for (int dy = -1; dy <= 1; dy++) {    // change here, dx dy from -1
    for (int dx = -1; dx <= 1; dx++) {  //change here // to 0
      int nx = x + dx;
      int ny = y + dy;  // line below
      if ((nx >= 0 && nx < b->width) && (ny >= 0 && ny < b->height)) {
  ...
  assert(knownMines + unknownSquares >= b->board[y][x]);  // failing here
