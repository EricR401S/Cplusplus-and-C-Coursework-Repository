...
std::pair<size_t, size_t> optimized_page_search(size_t page_number,
  ...
  // Returns the start and end ranges for a
  // for loops
  ...
  if (some_page_count > 10 && page_number >= some_page_count / 2) {
    ...
    // std::cout << "Searching for -" << page_number << "- between " << start_range
    //           << " and " << end_range << "\n";
  }
  ...
  // else {
  //   std::cout << "No search required for page -" << page_number << "-\n";
  // }
...
//     if (!std::cin.good()) {
//       std::cout << "Will this write again?\n";
...
//     else {
//       if (user_input >= 1 && user_input <= options_count) {
//         number_found = true;
//       }
//       else {
//         std::cout << "That is not a valid choice, please try again\n";
...
//   std::cout << "This is only wspace.That is not a valid choice, please try again\n";
//   continue;
// }
...
void print_path(std::pair<std::vector<size_t>, std::set<size_t> > & path) {
  // Prints the paths contents
  std::cout << "My first item in the path is the path vector, and it has a size of -"
            << path.first.size() << "- and the contents are-";
  ...
  std::cout << "The set's contents are of size -" << path.second.size()
            << "- and the contents are -";

  for (std::set<size_t>::iterator it = path.second.begin(); it != path.second.end();
       it++) {
    std::cout << *it << " ,";
  }
...
void print_path_queue(
    std::vector<std::pair<std::vector<size_t>, std::set<size_t> > > & path_queue) {
  // Prints the contents of the path queue

  std::cout << "This is the current path queue\n";

  for (size_t i = 0; i < path_queue.size(); i++) {
    std::cout << "Path -" << i << "- : \n";

    print_path(path_queue[i]);
  }
}
...
void funnel_win_paths(
  ...
  for (size_t j = 0; j < path_removal_list.size(); j++) {
    //std::cout << "This is my path to erase: \n";
    //print_path(path_queue[path_removal_list[j] - j]);
    // std::cout << "Commence erase\n";
...
void find_next_paths(
  ...
  for (size_t i = 0; i < updated_path_to_remove; i++) {
    ...
    //std::cout if something goes wrong
    ...
    for (size_t j = 0; j < page_choices.size(); j++) {
      ...
      //     path_queue[i].second.insert(next_page_destination);
      ...
      if (ret.second == false) {
        ...
        // std::cout << "Going from -" << last_page_n_in_path << "- to -"
        //           << next_page_destination << "-\n Active path : ";
        // print_path(path_queue[i]);
        // std::cout << "End of failed duplication examination\n";
  ...
  //print_path_queue(path_queue);
  //std::cout << "\nAfter removing old paths:\n";
  path_queue.erase(path_queue.begin(), path_queue.begin() + updated_path_to_remove);
  //print_path_queue(path_queue);
  //std::cout << "done with lap\n";
}
...
void find_winning_paths(Story & the_story) {
  ...
  //std::vector<std::vector<size_t> > winning_paths;
  ...
  // std::cout << "My path queueu has a size of -" << path_queue.size() << "-\n";
  // std::cout << "My first item in the queueu (and only item) is comprised of both a "
  //              "vector and a set. The vector is of size -"
  //           << path_queue[0].first.size() << "- and the contents are-"
  //           << path_queue[0].first[0] << "-\n";
  // std::cout << "The sets contents are of size -" << path_queue[0].second.size()
  //           << "- and the contents are -";

  // for (std::set<size_t>::iterator it = path_queue[0].second.begin();
  //      it != path_queue[0].second.end();
  //      it++) {
  //   std::cout << *it << " ";
  // }

  // std::cout << "-\n";
  // std::cout << finished << "First demo complete\n";
  ////////////////////////////////
  // find_next_paths(path_queue, the_story);

  // funnel_win_paths(path_queue, winning_paths, the_story);

  // std::cout << "\nPrinting queue after full first pass - 0 to next pages - \n";
  // print_path_queue(path_queue);

  // std::cout << "\nPrinting winning paths\n";
  // print_path_queue(winning_paths);
  ...
  while (path_queue.size() != 0) {
    ...
    //std::cout << "\nPrinting queue after full pass , iteration -" << count << "- \n";
    //print_path_queue(path_queue);
  ...
  //std::cout << "\nPrinting the winning paths now:\n";
...
bool digit_checker_long_int(std::string some_line, size_t equal_start, size_t end) {
  ...
  size_t sz = end;  //some_line.size();

  // to handle a case like 4$pastry=2, one positive number after the equal sign
  if (sz - equal_start - 1 == 1) {
    ...
    // track if the first character after the equal sign is a minus sign
...
bool variable_check(std::string story_line) {
  ...
  size_t equal_location = story_line.find('=');
  ...
  // what if the declaration lacks a variable name
  // such as 4$=1 instead of 4$pastry=1
  // This assumes also that no valid cases look like
  // 4$=pastry==1, where =pastry= is the variable name

  if (story_line.substr($_location + 1, 1) == "=") {
    return false;
  }
...
std::pair<size_t, std::pair<std::string, long int> > variable_parser(
  ...
  // Returns a pair consisting of the variable name and the value as a long integet
  ...
  //std::cout << "The page number in the variable parser is" << page_number << "\n";
  ...
  // change return eventually
  // pass story as an argument
  // add the key and value to the map and aux map.
...
bool conditional_choice_checker(std::string story_line) {
  ...
  //size_t location_right_bracket = backwards_location_char_finder(story_line, ']');
  ...
  // std::cout << "My conditional choice string is -" << story_line << "-\nMy variable -"
  //           << variable_string << "- has the name -" << var_name << "- with the number -"
  //           << var_number << "- and the choice string itself is -" << choice_string
  //           << "-\n";
