...
void numToBits(uint32_t * nums, int nNums, int * bits, int nBits) {
  if (nBits < nNums * 32) {
    ...
    // nums, as an array, is provided for us.
    ...
    for (int i = 0; i < nNums; i++) {
      for (int j = 0; j <= bit_counter; j++) {
        bits[start_index + j] = getNthBit(nums[i], bit_counter - j);
      }

      last_index = start_index + bit_counter;  // imagine 0 + 31 or 32 + 31 to get 63

      start_index = last_index + 1;  // imagine 31 + 1 to start the next number i
    }
    return;
  }
}
