...
board_t * makeBoard(int w, int h, int numMines) {
  // WRITE ME!
  // Function that makes the board with the appropriate
  // dimensions in malloced memory

  // malloc a pointer to the struct

  board_t * b = malloc(1 * sizeof(board_t));
  ...
  b->board = malloc(h * sizeof(*b->board));  // *((*b).board) - int *

  for (int i = 0; i < h; i++) {
    b->board[i] =
        malloc(w * sizeof(*(b->board[i])));  // not sure about what it dereferences
  ...
  for (int k = 0; k < numMines; k++) {
    addRandomMine(b);  // arg is board_t * b
  }

  return b;  // used to be NULL
}
...
int countMines(board_t * b, int x, int y) {
  ...
  for (int dy = -1; dy <= 1; dy++) {
    for (int dx = -1; dx <= 1; dx++) {
      ...
      else if (nx >= 0 && nx < b->width && ny >= 0 && ny < b->height) {
...
void freeBoard(board_t * b) {
  // WRITE ME!
  // function to release all board memory
  for (int i = 0; i < b->height; i++) {
    free(b->board[i]);  // the individual arrays, rows
  }
  free(b->board);
  free(b);
  return;
